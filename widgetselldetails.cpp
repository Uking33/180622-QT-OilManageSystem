...
WidgetSellDetails::WidgetSellDetails(OutPut *m_output, WingWidget *m_parent, int index)
    :WingWidget(m_output, m_parent)
{
    this->setWindowTitle("销售-详情");
    m_index=index;
    m_type = 1;
    m_count = 0;
    m_isDetails = true;
    m_findLabels.append(m_output->m_findLabels4);
...
WidgetSellDetails::~WidgetSellDetails(){
...
void WidgetSellDetails::CreateNew(){
    m_editWidget1=NULL;
    m_editWidget2=NULL;
    m_upEditWidget=NULL;
    //按钮
...
void WidgetSellDetails::SetDate(QString str1,QString str2){
    m_dateSBox->Update(str1);
    m_dateEBox->Update(str2);
}

void WidgetSellDetails::ButtonFindPress(){
    //m_table
    m_count = WingWidget::BtnFind(this->m_output->vars4[m_index], m_table, m_tableType2, 8);
    //m_table_up
    QList<QList<QVariant>> list2;
    list2.append(this->m_output->vars3[0]);
    list2.append(this->m_output->vars3[m_index]);
    this->m_output->VarToTable(list2, m_table_up, m_tableType1, 1);
}
void WidgetSellDetails::ButtonAddPress(){
    if(m_editWidget1 == NULL){
        m_editWidget1 = new WidgetSellDetailsEdit(m_output,this,m_index);
...
void WidgetSellDetails::ButtonEditPress(){
    if(m_table->hasFocus()){
        if(m_table->currentRow()<m_count){
            int index=m_indexList[m_table->currentRow()+1];
            if(m_editWidget2 == NULL){
                m_editWidget2 = new WidgetSellDetailsEdit(m_output,this,m_index,index);
        ...
        if(m_upEditWidget == NULL){
            m_upEditWidget = new WidgetSellEdit(m_output,this,m_index,true);
...
void WidgetSellDetails::ButtonDelPress(){
    QList<int>* indexList=WingWidget::BtnDel(m_count);
    for(int i=indexList->count()-1;i>=0;i--)
        if(indexList->at(i)<m_count){
            m_output->vars4[m_index].removeAt(indexList->at(i)+1);
        }
    DelSum(indexList, m_index, &m_output->vars3, &m_output->vars4, m_output->m_tableAuto4, m_output->m_tableType4);
    ButtonFindPress();
    delete indexList;
}
void WidgetSellDetails::ButtonOutputPress(){
    QString str;
    if(m_output->MessageQuestion("导出当前已搜索的内容，单独导出一个文件")){
        ...
        if (!str.isNull()){
            ...
            for(int i=0;i<m_indexList.count();i++)
                list.append(m_output->vars4[m_index][m_indexList.at(i)]);
