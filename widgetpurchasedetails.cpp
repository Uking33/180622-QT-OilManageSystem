...
WidgetPurchaseDetails::WidgetPurchaseDetails(OutPut *m_output, WingWidget *m_parent, int index)
    :WingWidget(m_output, m_parent)
{
    this->setWindowTitle("采购-详情");
    m_index=index;
    m_type = 1;
    m_count = 0;
    m_isDetails = true;
    m_findLabels.append(m_output->m_findLabels2);
...
WidgetPurchaseDetails::~WidgetPurchaseDetails(){
...
void WidgetPurchaseDetails::CreateNew(){
    //Init
    m_editWidget1=NULL;
    m_editWidget2=NULL;
    m_upEditWidget=NULL;
...
void WidgetPurchaseDetails::SetDate(QString str1,QString str2){
    m_dateSBox->Update(str1);
    m_dateEBox->Update(str2);
}
void WidgetPurchaseDetails::ButtonFindPress(){
    //m_table
    m_count = WingWidget::BtnFind(this->m_output->vars2[m_index], m_table, m_tableType2, 8);
    //m_table_up
    QList<QList<QVariant>> list2;
    list2.append(this->m_output->vars1[0]);
    list2.append(this->m_output->vars1[m_index]);
    this->m_output->VarToTable(list2, m_table_up, m_tableType1, 1);
}
void WidgetPurchaseDetails::ButtonAddPress(){
    if(m_editWidget1 == NULL){
        m_editWidget1 = new WidgetPurchaseDetailsEdit(m_output,this,m_index);
...
void WidgetPurchaseDetails::ButtonEditPress(){
    if(m_table->hasFocus()){
        if(m_table->currentRow()<m_count){
            int index=m_indexList[m_table->currentRow()+1];
            if(m_editWidget2 == NULL){
                m_editWidget2 = new WidgetPurchaseDetailsEdit(m_output,this,m_index,index);
        ...
        if(m_upEditWidget == NULL){
            m_upEditWidget = new WidgetPurchaseEdit(m_output,this,m_index,true);
...
void WidgetPurchaseDetails::ButtonDelPress(){
    QList<int>* indexList=WingWidget::BtnDel(m_count);
    for(int i=indexList->count()-1;i>=0;i--)
        if(indexList->at(i)<m_count){
            m_output->vars2[m_index].removeAt(indexList->at(i)+1);
        }
    DelSum(indexList, m_index, &m_output->vars1, &m_output->vars2, m_output->m_tableAuto2, m_output->m_tableType2);
    ButtonFindPress();
    delete indexList;
}
void WidgetPurchaseDetails::ButtonOutputPress(){
    QString str;
    if(m_output->MessageQuestion("导出当前已搜索的内容，单独导出一个文件")){
        ...
        if (!str.isNull()){
            ...
            for(int i=0;i<m_indexList.count();i++)
                list.append(m_output->vars2[m_index][m_indexList.at(i)]);
